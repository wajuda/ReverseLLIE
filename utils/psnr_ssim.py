...
def calculate_tuple(result_tuple, target_tuple, loss_function):
    l = len(result_tuple)
    loss=[]
    if loss_function == 'psnr':
        for i in range(l):
            loss.append(calculate_psnr(result_tuple[i], target_tuple[i]))
    elif loss_function == 'ssim':
        for i in range(l):
            loss.append(calculate_ssim(result_tuple[i], target_tuple[i]))
    return np.mean(loss)
